...
def callback_function_for_zcjd(res, **kwargs):
    ...
    try:
        ...
        trs = soup.find(name='table', attrs={'id': 'id_bl'}).find_all(name='tr')
        for tr in trs:
            link = tr.find(name='a')['href'].strip()
            if link.startswith('./'):
            ...
            title = tr.find(name='td', attrs={'class': 'ZITI'})['title'].strip()
            date = title[-12:][1:][:-1]
            title = title[:-13]
...
def main():
    ...
    # zcjd_request_urls = ['http://www.mof.gov.cn/zhengwuxinxi/zhengcejiedu/index_%d.htm' % j for j in range(1, num_of_pages)]
